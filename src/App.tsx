...
import { runOrbit } from './orbitEngine';
import {
  Send,
  Users, 
  Clock, 
...
export default function App() {
    ...
    orbit: [
      {
        id: 'o1',
        role: 'assistant',
        content: `Greetings! I am **Orbit**, your systematic logistics copilot. I run on a live local event database — no API key needed. 🛰️\n\nAsk me about the **schedule**, **next event**, **room directions**, **parking**, **announcements**, **dining menu**, or **attendee stats** — or set a **reminder**.\n\n🛠️ **Organizers can edit event info right here**, e.g.:\n\`add event: Demo Party | 03:00 PM | Day 2 | 1650 | Main Hall\`\n\`announce: 12:45 PM | Lunch is now open!\`\n\`set menu: Dinner | Day 1 | Sushi platters & ramen\`\n\`set stats: registered=320 teams=66\``,
        timestamp: new Date()
      }
    ],
    sage: [
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    // Orbit runs entirely client-side on the local hackathon database (no server / API key
    // needed). It answers logistics questions, edits event info, and can open the reminder
    // scheduler — see src/orbitEngine.ts.
    if (activeAgent === 'orbit') {
      await new Promise(r => setTimeout(r, 650)); // brief "thinking" beat for the typing animation
      const { content, reminderConfig } = runOrbit(userMessageText || '');
      ...
      setChats(prev => ({ ...prev, orbit: [...prev.orbit, newResponse] }));
      if (reminderConfig) {
        setPendingReminder({
          title: reminderConfig.title,
          time: reminderConfig.time,
          location: reminderConfig.location || 'Venue',
          channel: 'email',
          recipient: '',
          msgId: aiMsgId
        });
        addToast(`Orbit drafted a reminder for "${reminderConfig.title}"!`, 'info');
      }
      setIsLoading(false);
      return;
    }
              ...
              <div className="flex gap-2.5 mt-2.5 flex-nowrap overflow-x-auto pb-0.5 justify-start shrink-0">
                {activeAgent === 'luna' ? (
                  ...
                  <>
                    <button
                      type="button"
                      onClick={() => setInputText("What's the schedule for Day 1?")}
                      className={`text-[9.5px] font-bold ${theme.quickBtn} hover:bg-white hover:scale-105 rounded-full px-3 py-1 cursor-pointer shrink-0 transition-all shadow-sm`}
                    >
                      🗓️ Day 1 Schedule
                    </button>
                    <button
                      type="button"
                      onClick={() => setInputText("Remind me about the Gemini API workshop.")}
                      className={`text-[9.5px] font-bold ${theme.quickBtn} hover:bg-white hover:scale-105 rounded-full px-3 py-1 cursor-pointer shrink-0 transition-all shadow-sm`}
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => setInputText("announce: 12:45 PM | Lunch is now open in the Dining Area!")}
                      className={`text-[9.5px] font-bold ${theme.quickBtn} hover:bg-white hover:scale-105 rounded-full px-3 py-1 cursor-pointer shrink-0 transition-all shadow-sm`}
                    >
                      🛠️ Post Announcement
                    </button>
