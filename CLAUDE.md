...
### Agents: Luna & Sage on the server, Orbit on the client
**Luna** (teammate matcher) and **Sage** (perk discovery) are distinguished by the `agentId` field on the `/api/chat` request, which selects a hardcoded `systemInstruction` string in [server.ts](server.ts). Each prompt embeds the relevant mock data (`TEAMMATES` or `PERKS`) as JSON so the model can reference real IDs. **To change Luna/Sage behavior, edit the prompt strings in `server.ts`.**

**Orbit** (logistics) does **NOT** use the server. It was merged in from an earlier vanilla app and now runs entirely client-side in [src/orbitEngine.ts](src/orbitEngine.ts): a `localStorage`-backed hackathon database (schedule, locations, parking, announcements, dining, stats) with read tools, write/update tools (organizers edit event info via `add event:` / `announce:` / `set menu:` etc. commands), keyword-based intent matching, and a reminder-intent that returns a `reminderConfig`. `App.tsx`'s `handleSendMessage` short-circuits `activeAgent === 'orbit'` to `runOrbit()` and never calls `/api/chat`. So Orbit works with **no `GEMINI_API_KEY`**. **To change Orbit behavior or data, edit `src/orbitEngine.ts`.**
