...
- `POST /api/seed` — seeds the 391 candidate profiles (with embeddings); no-op if present.
...
- `POST /api/reminders/trigger` — manual/fallback path (the in-chat scheduler widget): fires a reminder NOW (persists a `reminders` row + invokes the Lambda for email/telegram, else logs simulated). `GET /api/reminders/{userId}` lists a user's reminders (powers the Orbit "Lịch nhắc của tôi" history panel). `DELETE /api/reminders/{reminderId}` cancels a pending reminder (deletes its EventBridge schedule via `aws_scheduler.delete_schedule` + marks the row `cancelled`). `GET /api/health`.
